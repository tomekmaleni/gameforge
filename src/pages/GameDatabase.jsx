...
import { Plus, Pencil, Trash2, Database, Upload, X } from 'lucide-react';
...
export default function GameDatabase() {
  ...
  const SORT_OPTIONS = [{ value: 'newest', label: 'Newest First' }, { value: 'oldest', label: 'Oldest First' }, { value: 'category', label: 'By Category' }, { value: 'mechanic', label: 'By Mechanic' }];
          ...
          {displayedEntries.length === 0 ? (<p className="text-slate-500 text-center py-12">{categories.length === 0 ? 'Create a category to start adding entries.' : 'No entries found.'}</p>) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {displayedEntries.map(entry => (
                <div key={entry.id} onClick={() => setSelectedEntry(entry)} className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden cursor-pointer hover:border-amber-500/30 transition-all flex items-center gap-3 p-3">
                  ...
                  <div className="flex-1 min-w-0"><h3 className="font-semibold text-slate-100 truncate">{entry.name}</h3>{entry.data?.Type && (<span className="inline-block text-xs bg-amber-500/20 text-amber-400 border border-amber-500/30 rounded px-1.5 py-0.5 mt-0.5 mb-1">{entry.data.Type}</span>)}{entry.mechanic_ids?.length > 0 && (<p className="text-xs text-slate-500">⚙ {entry.mechanic_ids.map(id => mechanics.find(m => m.id === id)?.name).filter(Boolean).join(', ')}</p>)}{entry.tags?.length > 0 && (<div className="flex flex-wrap gap-1 mt-1">{entry.tags.map(t => <Badge key={t} variant="outline" className="text-xs border-slate-700 text-slate-400">#{t}</Badge>)}</div>)}</div>
                </div>
              ))}
            </div>
          )}
      ...
      <Dialog open={showEntryDialog} onOpenChange={(open) => { if (!open) { setShowEntryDialog(false); resetEntryForm(); } }}>
        <DialogContent className="bg-slate-900 border-slate-800 text-slate-100 max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-3">
            ...
            {entryFormFields.map(f => (<div key={f.name}><label className="text-xs text-slate-500 mb-1 block">{f.name}</label>{f.type === 'textarea' ? (<Textarea value={entryData[f.name] || ''} onChange={e => setEntryData({ ...entryData, [f.name]: e.target.value })} className="bg-slate-800 border-slate-700" />) : (<Input value={entryData[f.name] || ''} onChange={e => setEntryData({ ...entryData, [f.name]: e.target.value })} className="bg-slate-800 border-slate-700" />)}</div>))}

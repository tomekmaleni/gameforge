...
export default function IdeaBacklog() {
  ...
  const saveIdea = useMutation({
    mutationFn: ({ isEdit, editId, title: t, description: d, status: s }) => { if (isEdit) return base44.entities.Idea.update(editId, { title: t, description: d, status: s }); return base44.entities.Idea.create({ project_id: projectId, title: t, description: d, status: s }); },
    onSuccess: (_, { isEdit }) => { qc.invalidateQueries({ queryKey: ['ideas', projectId] }); closeForm(); toast.success(isEdit ? 'Idea updated' : 'Idea added'); },
